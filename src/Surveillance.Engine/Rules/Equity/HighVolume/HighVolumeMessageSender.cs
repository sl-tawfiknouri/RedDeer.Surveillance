    ...
    public class HighVolumeMessageSender : BaseMessageSender, IHighVolumeMessageSender
    {
        ...
        private string BuildDescription(IHighVolumeRuleBreach ruleBreach)
        {
            ...
            if (ruleBreach.DailyBreach.HasBreach)
            {
                ...
                dailyDescription =
                    $" Percentage of daily volume breach has occured. A daily volume limit of {dailyPercentage}% was exceeded by trading {dailyBreachPercentage}% of daily volume. {ruleBreach.TotalOrdersTradedInWindow} volume was ordered against a breach threshold volume of {ruleBreach.DailyBreach.BreachThresholdAmount}.";
            }
            ...
            if (ruleBreach.WindowBreach.HasBreach)
            {
                ...
                windowDescription =
                    $" Percentage of window volume breach has occured. A window volume limit of {windowPercentage}% was exceeded by trading {windowBreachPercentage}% of window volume within the window of {ruleBreach.EquitiesParameters.Windows.BackwardWindowSize.TotalMinutes} minutes. {ruleBreach.TotalOrdersTradedInWindow} volume was ordered against a breach threshold volume of {ruleBreach.WindowBreach.BreachThresholdAmount}.";
            }
            ...
            if (ruleBreach.MarketCapBreach.HasBreach)
            {
                ...
                marketCapDescription =
                    $" Percentage of market capitalisation breach has occured. A limit of {marketCapPercentage}% was exceeded by trading {marketCapBreachPercentage}% of market capitalisation.  ({ruleBreach.MarketCapBreach.BreachTradedMoney.Currency.Code}) {tradedAmount} was traded against a breach threshold value of ({ruleBreach.MarketCapBreach.BreachThresholdMoney.Currency.Code}) {thresholdAmount}.";
            }
