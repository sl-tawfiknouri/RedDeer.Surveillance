...
namespace Surveillance.DataLayer.Tests.ElasticSearch.Rules
{
    [TestFixture]
    public class RuleBreachRepositoryTests
    {
        ...
        public async Task Save_NullDocument_DoesNothing()
        {
            ...
            A
                .CallTo(() =>
                _dataAccess.GetOrCreateDateBasedIndexAsync<ReddeerTradeDocument>(A<string>.Ignored, A<DateTime>.Ignored,
                    A<CancellationToken>.Ignored))
                .MustNotHaveHappened();

            A
                .CallTo(() =>
                    _dataAccess.IndexDocumentAsync(A<string>.Ignored, A<ReddeerTradeDocument>.Ignored, A<DateTime>.Ignored, A<CancellationToken>.Ignored))
                .MustNotHaveHappened();
        }

        [Test]
        public async Task Save_Document_CreatesIndexAndThenSaves()
        {
            var repo = new RedDeerTradeFormatRepository(_dataAccess);
