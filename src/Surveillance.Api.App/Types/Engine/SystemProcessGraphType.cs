...
namespace Surveillance.Api.App.Types.Engine
{
    public class SystemProcessGraphType : ObjectGraphType<ISystemProcess>
    {
        public SystemProcessGraphType()
        {
            ...
            Field(i => i.Heartbeat, nullable: true).Type(new DateTimeGraphType()).Description("Heartbeat (UTC) in UK time format. The last time the process wrote to the database via polling. This is a substitute for process end time");
            Field(i => i.InstanceInitiated, nullable: true).Type(new DateTimeGraphType()).Description("Instance of the process was initiated at (UTC) in UK time formats");
