namespace DataImport.Disk_IO.Shared
{
    ...
    public abstract class BaseUploadOrderFileProcessor : BaseUploadFileProcessor<OrderFileContract, Order>
    {
        ...
        protected override OrderFileContract MapToCsvDto(CsvReader rawRecord, int rowId)
        {
            if (rawRecord == null)
            {
                this.Logger.LogInformation($"Upload Trade File Processor received a null record to map with row id {rowId}");
                return null;
            }
            ...
            var tradeCsv = new OrderFileContract
            {
                ...
                InstrumentIssuerIdentifier = this.PreProcess(rawRecord["InstrumentIssuerIdentifier"]),
                InstrumentClientIdentifier = this.PreProcess(rawRecord["InstrumentClientIdentifier"]),
                ...
                InstrumentUnderlyingIssuerIdentifier = this.PreProcess(rawRecord["InstrumentUnderlyingIssuerIdentifier"]),
                InstrumentUnderlyingClientIdentifier = this.PreProcess(rawRecord["InstrumentUnderlyingClientIdentifier"]),
                ...
                InstrumentUnderlyingExchangeSymbol = this.PreProcess(rawRecord["InstrumentUnderlyingExchangeSymbol"]),
                InstrumentUnderlyingBloombergTicker = this.PreProcess(rawRecord["InstrumentUnderlyingBloombergTicker"]),
                ...
                OrderOptionEuropeanAmerican = this.PreProcess(rawRecord["OrderOptionEuropeanAmerican"]),
                ...
                DealerOrderSettlementCurrency = this.PreProcess(rawRecord["DealerOrderSettlementCurrency"]),
                DealerOrderCleanDirty = this.PreProcess(rawRecord["DealerOrderCleanDirty"]),
                DealerOrderAccumulatedInterest = this.PreProcess(rawRecord["DealerOrderAccumulatedInterest"]),
                DealerOrderLimitPrice = this.PreProcess(rawRecord["DealerOrderLimitPrice"]),
                DealerOrderAverageFillPrice = this.PreProcess(rawRecord["DealerOrderAverageFillPrice"]),
                ...
                DealerOrderOptionStrikePrice = this.PreProcess(rawRecord["DealerOrderOptionStrikePrice"]),
                DealerOrderOptionExpirationDate =  this.PreProcess(rawRecord["DealerOrderOptionExpirationDate"]),
                DealerOrderOptionEuropeanAmerican = this.PreProcess(rawRecord["DealerOrderOptionEuropeanAmerican"]),
                RowId = rowId
            };
