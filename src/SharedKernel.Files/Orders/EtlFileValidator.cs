    ...
    public class EtlFileValidator : AbstractValidator<OrderFileContract>, IEtlFileValidator
    {
        ...
        private void RulesForDealerOrderProperties()
        {
            ...
            this.RuleFor(x => x.DealerOrderCurrency).Length(3).When(this.HasDealerOrderData)
                .WithMessage("DealerOrderCurrency must have a length of 3 characters");
            ...
            this.RuleFor(x => x.DealerOrderCleanDirty)
                ...
                .When(x => !string.IsNullOrWhiteSpace(x.DealerOrderCleanDirty)).WithMessage(
                    "DealerOrderCleanDirty was not recognised as a valid value. It is a closed set");
            ...
            this.RuleFor(x => x.DealerOrderType).NotEmpty()
                ...
                .WithMessage("DealerOrderType was not recognised as a valid value. It is a closed set");
            this.RuleFor(x => x.DealerOrderDirection).NotEmpty()
                .SetValidator(new EnumParseableValidator<OrderDirections>("TradePosition"))
                .When(this.HasDealerOrderData).WithMessage(
                    "DealerOrderDirection was not recognised as a valid value. It is a closed set");
            ...
            this.RuleFor(x => x.DealerOrderLimitPrice).SetValidator(new DecimalParseableValidator("TradeLimitPrice"))
                .When(this.HasDealerOrderData).WithMessage(
                    "DealerOrderLimitPrice must be provided when there is a dealer order type of LIMIT");
            this.RuleFor(x => x.DealerOrderAverageFillPrice)
                ...
                .WithMessage("DealerOrderAveragePrice was not recognised as a valid decimal");
            this.RuleFor(x => x.DealerOrderOrderedVolume)
                ...
                .WithMessage("DealerOrderOrderedVolume was not recognised as a valid non zero long (64 bit integer)");
            this.RuleFor(x => x.DealerOrderFilledVolume)
                ...
                .WithMessage("DealerOrderFilledVolume was not recognised as a valid nullable long (64 bit integer)");
            this.RuleFor(x => x.DealerOrderAccumulatedInterest)
                .SetValidator(new DecimalParseableValidator("AccumulatedInterest"))
                .When(x => !string.IsNullOrWhiteSpace(x.DealerOrderAccumulatedInterest)).WithMessage(
                    "DealerOrderAccumulatedInterest was not recognised as a valid decimal");
            ...
            this.RuleFor(x => x.DealerOrderPlacedDate).NotEmpty().When(this.HasDealerOrderData)
                .WithMessage("DealerOrderPlacedDate must not be empty");
            this.RuleFor(x => x.DealerOrderPlacedDate).SetValidator(new DateParseableValidator("DealerOrderPlacedDate"))
                .When(this.HasDealerOrderData).WithMessage("DealerOrderPlacedDate was not recognised as a valid date");
            this.RuleFor(x => x.DealerOrderBookedDate).SetValidator(new DateParseableValidator("DealerOrderBookedDate"))
                .When(this.HasDealerOrderData).WithMessage("DealerOrderBookedDate was not recognised as a valid date");
            this.RuleFor(x => x.DealerOrderAmendedDate)
                ...
                .WithMessage("DealerOrderAmendedDate was not recognised as a valid date");
            this.RuleFor(x => x.DealerOrderCancelledDate)
                ...
                .WithMessage("DealerOrderCancelledDate was not recognised as a valid date");
            this.RuleFor(x => x.DealerOrderRejectedDate)
                ...
                .WithMessage("DealerOrderRejectedDate was not recognised as a valid date");
            this.RuleFor(x => x.DealerOrderFilledDate).SetValidator(new DateParseableValidator("DealerOrderFilledDate"))
                .When(this.HasDealerOrderData).WithMessage("DealerOrderFilledDate was not recognised as a valid date");
        }
        ...
        private void RulesForOrderProperties()
        {
            ...
            this.RuleFor(x => x.OrderAverageFillPrice).SetValidator(new DecimalParseableValidator("OrderAveragePrice"))
                .WithMessage("Order average price was not a valid value");
            this.RuleFor(x => x.OrderFilledVolume).SetValidator(new DecimalParseableValidator("OrderFilledVolume"))
                .WithMessage("Order filled volume was not a valid long value (64 bit integer)");
            ...
            this.RuleFor(x => x.OrderAccumulatedInterest)
                ...
                .When(x => !string.IsNullOrWhiteSpace(x.OrderAccumulatedInterest)).WithMessage(
                    "Order accumulated interest was not recognised as a parseable decimal value");

            this.RuleFor(x => x.OrderPlacedDate).NotEmpty().WithMessage("OrderPlacedDate must not be empty");
            this.RuleFor(x => x.OrderPlacedDate).SetValidator(new DateParseableValidator("OrderPlacedDate"))
                .WithMessage("OrderPlacedDate was not recognised as a valid date");
            this.RuleFor(x => x.OrderBookedDate).SetValidator(new DateParseableValidator("OrderBookedDate"))
                .WithMessage("OrderBookedDate was not recognised as a valid date");
            this.RuleFor(x => x.OrderAmendedDate).SetValidator(new DateParseableValidator("OrderAmendedDate"))
                .WithMessage("OrderAmendedDate was not recognised as a valid date");
            this.RuleFor(x => x.OrderRejectedDate).SetValidator(new DateParseableValidator("OrderRejectedDate"))
                .WithMessage("OrderRejectedDate was not recognised as a valid date");
            this.RuleFor(x => x.OrderCancelledDate).SetValidator(new DateParseableValidator("OrderCancelledDate"))
                .WithMessage("OrderCancelledDate  was not recognised as a valid date");
            this.RuleFor(x => x.OrderFilledDate).SetValidator(new DateParseableValidator("OrderFilledDate"))
                .WithMessage("OrderFilledDate was not recognised as a valid date");
