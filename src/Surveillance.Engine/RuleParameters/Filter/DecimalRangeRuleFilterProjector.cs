namespace Surveillance.Engine.Rules.RuleParameters.Filter
{
    ...
    public class DecimalRangeRuleFilterProjector : IDecimalRangeRuleFilterProjector
    {
        ...
        {
            if (filter == null) return DecimalRangeRuleFilter.None();

            if (filter.LowerBoundary == null && filter.UpperBoundary == null) return DecimalRangeRuleFilter.None();

            return new DecimalRangeRuleFilter
                       {
                           Min = filter.LowerBoundary, Max = filter.UpperBoundary, Type = RuleFilterType.Include
                       };
        }
    }
}