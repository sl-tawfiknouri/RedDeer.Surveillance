namespace DataSynchroniser.Api.Bmll.Bmll
{
    ...
    public class
        MarketDataRequestToMinuteBarRequestKeyDtoProjector : IMarketDataRequestToMinuteBarRequestKeyDtoProjector
    {
        public IReadOnlyCollection<MinuteBarRequestKeyDto> ProjectToRequestKeys(IList<MarketDataRequest> bmllRequests)
        {
            if (bmllRequests == null || !bmllRequests.Any()) return new List<MinuteBarRequestKeyDto>();

            var projectKeys = this.Project(bmllRequests);
            var filteredKeys = projectKeys.Where(key => !string.IsNullOrWhiteSpace(key.Figi)).ToList();
            var deduplicatedKeys = this.DeduplicateKeys(filteredKeys);
        ...
        private IList<MinuteBarRequestKeyDto> Project(IList<MarketDataRequest> bmllRequests)
        {
            ...
            foreach (var req in bmllRequests)
            {
                if (req == null || string.IsNullOrWhiteSpace(req.Identifiers.Figi) || req.UniverseEventTimeTo == null
                    || req.UniverseEventTimeFrom == null) continue;
