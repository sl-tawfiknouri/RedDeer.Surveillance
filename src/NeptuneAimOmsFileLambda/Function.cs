...
namespace AimOmsFileHeaderTrimmerLambda
{
    ...
    public class Function
    {
        ...
        {
            LambdaLogger.Log(
                $"Function handler for aim oms file lambda invoked. Invoked function arn - {context.InvokedFunctionArn}");
            LambdaLogger.Log(
                $"Function invoked for event {evnt.Records.FirstOrDefault().EventName} {evnt.Records.FirstOrDefault().EventSource}");
            ...
            LambdaLogger.Log(
                $"Function handler for aim oms file lambda invoked. This is the output bucket {aimOmsKeyBucket} and this is the output key path {aimOmsKeyPath}");
            ...
            {
                LambdaLogger.Log(
                    $"S3 event get object request built {s3Event.Bucket.Name} target key {s3Event.Object.Key}");
                var getObjectRequest =
                    new GetObjectRequest { BucketName = s3Event.Bucket.Name, Key = s3Event.Object.Key };
                ...
                LambdaLogger.Log(
                    $"S3 event get object response bytes written to a memory stream {memStream.Length} length");
                ...
                LambdaLogger.Log(
                    $"S3 finished the put object request. Had status code {putObjectResponse.HttpStatusCode}");
            ...
            catch (Exception e)
            {
                context.Logger.LogLine(
                    $"Error getting object {s3Event.Object.Key} from bucket {s3Event.Bucket.Name}. Make sure they exist and your bucket is in the same region as this function. Message {e.Message}. Stacktrace {e.StackTrace}");
                throw;
            }
        }
    }
