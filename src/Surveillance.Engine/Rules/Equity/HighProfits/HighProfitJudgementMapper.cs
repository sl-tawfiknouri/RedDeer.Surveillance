...
namespace Surveillance.Engine.Rules.Rules.Equity.HighProfits
{
    public class HighProfitJudgementMapper : IHighProfitJudgementMapper
    {
        ...
        public IRuleBreach Map(IHighProfitJudgementContext judgementContext)
        {
            ...
            {
                _logger?.LogInformation($"{nameof(judgementContext)} was null in map. Returning.");
                return null;
            }
            ...
            var ruleBreach = 
                new RuleBreach(
                    judgementContext.RuleBreachContext,
                    description,
                    caseTitle);
        ...
        private string BuildDescription(IHighProfitJudgementContext judgementContext)
        {
            ...
            var highRelativeProfitSection =
                HighRelativeProfitText(judgementContext, highRelativeProfitAsPercentage, highRelativeProfitAsPercentageSetByUser);

            var highAbsoluteProfitSection = HighAbsoluteProfitText(judgementContext, highAbsoluteProfit);
            var highProfitExchangeRatesSection = HighProfitExchangeRateText(judgementContext);
            
            return $"High profit rule breach detected for {judgementContext.RuleBreachContext.Security.Name} at {judgementContext.RuleBreachContext.UniverseDateTime}.{highRelativeProfitSection}{highAbsoluteProfitSection}{highProfitExchangeRatesSection}";
        }
        ...
        private string HighProfitExchangeRateText(IHighProfitJudgementContext judgementContext)
        {
            ...
            {
                _logger.LogError($"HighProfitMessageSender had two equal currencies when generating WER text {judgementContext.ExchangeRateProfits.FixedCurrency.Code} and {judgementContext.ExchangeRateProfits.VariableCurrency.Code}");
                return string.Empty;
            }
