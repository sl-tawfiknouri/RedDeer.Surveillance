    ...
    public class CancelledOrderMessageSender : ICancelledOrderMessageSender
    {
            ...
            Domain.Trades.Orders.TradeOrderFrame anyOrder)
        {
            var positionSizeSegment = ruleBreach.ExceededPercentagePositionCancellations
                ? $" Position cancelled exceeded threshold at {ruleBreach.PercentagePositionCancelled * 100}% cancelled. Limit was set at {parameters.CancelledOrderPercentagePositionThreshold * 100}%."
                : string.Empty;

            var orderRatioSegment = ruleBreach.ExceededPercentageTradeCountCancellations
                ? $" Number of orders cancelled exceeded threshold at {ruleBreach.PercentageTradeCountCancelled * 100}% cancelled. Limit was set at {parameters.CancelledOrderCountPercentageThreshold * 100}%."
                : string.Empty;
