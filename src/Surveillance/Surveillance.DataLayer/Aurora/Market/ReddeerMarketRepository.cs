namespace Surveillance.DataLayer.Aurora.Market
{
    ...
    public class ReddeerMarketRepository : IReddeerMarketRepository
    {
        ...
        private const string GetUnEnrichedSecuritiesSql = @"SELECT 
              sec.Id AS Id,
        ...
        private const string UpdateUnEnrichedSecuritiesSql = @"UPDATE FinancialInstruments
        ...
        public void Create(EquityIntraDayTimeBarCollection entity)
        {
            if (entity == null)
            {
                this._logger.LogInformation("ReddeerMarketRepository create received a null entity");
            ...
            try
            {
                this._logger.LogInformation("ReddeerMarketRepository opened connection to the database");
                dbConnection.Open();

                if (!entity.Securities?.Any() ?? true)
                {
                    this._logger.LogInformation(
                        $"ReddeerMarketRepository did not detect any securities for {entity.Epoch} - {entity.Exchange?.MarketIdentifierCode}");
                    return;
                }
                ...
                this._logger.LogInformation(
                    $"ReddeerMarketRepository Create method about to write market match or insert sql for market {marketUpdate.MarketId} {marketUpdate.MarketName}");
                marketId = dbConnection.ExecuteScalar<string>(MarketMatchOrInsertSql, marketUpdate);
                this._logger.LogInformation(
                    $"ReddeerMarketRepository Create method finished writing market match or insert sql for market {marketUpdate.MarketId} {marketUpdate.MarketName} and fetched id of {marketId}");

                this._logger.LogInformation(
                    $"ReddeerMarketRepository about to write {entity.Securities?.Count} rows to database");
                foreach (var security in entity.Securities)
                {
                    var securityUpdate = new InsertSecurityDto(security, marketId, this._cfiMapper);
                    this._logger.LogInformation(
                        $"ReddeerMarketRepository about to write row to database {security.Security?.Identifiers} {security?.TimeStamp}");
                    dbConnection.Execute(SecurityMatchOrInsertSqlv2, securityUpdate);
                    this._logger.LogInformation(
                        $"ReddeerMarketRepository finished writing row to database {security.Security?.Identifiers} {security?.TimeStamp}");
                }
            }
            catch (Exception e)
            {
                this._logger.LogError(
                    $"ReddeerMarketRepository Create Method For {entity.Exchange?.Name} ERROR MESSAGE ({e.Message}) INNER ERROR MESSAGE ({e?.InnerException.Message})");
            }
            finally
            {
                this._logger.LogInformation("ReddeerMarketRepository closing db connection for create security");
        ...
        public async Task<MarketSecurityIds> CreateAndOrGetSecurityId(MarketDataPair pair)
        {
            ...
            {
                this._logger.LogError(
                    "Reddeer Market Repository CreateAndOrGetSecurityId was passed a null market data pair.");
            ...
            if (!string.IsNullOrWhiteSpace(pair.Security?.Identifiers.Id)
                ...
                return new MarketSecurityIds
                           {
                               MarketId = pair.Exchange.Id, SecurityId = pair.Security?.Identifiers.Id
                           };

            if (pair.Exchange == null || string.IsNullOrWhiteSpace(pair.Exchange?.MarketIdentifierCode))
            {
                this._logger.LogError(
                    "Reddeer Market Repository CreateAndOrGetSecurityId either received a null exchange object or an empty market id for a trade. Not able to continue.");
                return new MarketSecurityIds();
            }
            ...
            try
            {
                dbConnection.Open();
                this._logger.LogInformation(
                    "Reddeer Market Repository CreateAndOrGetSecurityId opened connection to database");
                ...
                this._logger.LogInformation(
                    $"Reddeer Market Repository CreateAndOrGetSecurityId about to match market or insert sql for {pair.Exchange?.MarketIdentifierCode}");
                using (var conn = dbConnection.ExecuteScalarAsync<string>(MarketMatchOrInsertSql, marketUpdate))
                {
                    marketId = await conn;
                    this._logger.LogInformation(
                        $"Reddeer Market Repository CreateAndOrGetSecurityId match market or insert sql for {pair.Exchange?.MarketIdentifierCode}");
                }

                var securityUpdate = new InsertSecurityDto(pair.Security, marketId, this._cfiMapper);

                this._logger.LogInformation(
                    $"Reddeer Market Repository CreateAndOrGetSecurityId about to match security or insert sql for {pair.Security?.Name}");
                using (var conn = dbConnection.ExecuteScalarAsync<string>(SecurityMatchOrInsertSql, securityUpdate))
                {
                    securityId = await conn;
                    this._logger.LogInformation(
                        $"Reddeer Market Repository CreateAndOrGetSecurityId completed match security or insert sql for {pair.Security?.Name}");
                }
            ...
            {
                this._logger.LogError(
                    $"ReddeerMarketRepository CreateAndOrGetSecurityId {e.Message} {e.InnerException?.Message}");
            }
            finally
            {
                this._logger.LogInformation(
                    "Reddeer Market Repository CreateAndOrGetSecurityId closed connection to database");
            ...
            ISystemProcessOperationContext opCtx)
        {
            ...
            if (start > end)
            {
                this._logger.LogWarning(
                    $"ReddeerMarketRepository GetEquityInterDay request had a start date larger than end date {start} {end}");
                return new EquityInterDayTimeBarCollection[0];
            }

            if (start == end) end = end.AddDays(1).AddMilliseconds(-1);
            ...
            {
                dbConnection.Open();
                this._logger.LogInformation("ReddeerMarketRepository GetEquityInterDay opened connection to db");

                this._logger.LogInformation(
                    $"ReddeerMarketRepository GetEquityInterDay about to query for {start} and {end}");
                using (var conn =
                    dbConnection.QueryAsync<MarketStockExchangeSecuritiesDto>(GetEquityInterDaySql, query))
                {
                    var response = (await conn)?.ToList() ?? new List<MarketStockExchangeSecuritiesDto>();

                    this._logger.LogInformation(
                        $"ReddeerMarketRepository GetEquityInterDay query for {start} and {end} received {response?.Count() ?? 0} results");

                    var groupedByExchange = response.GroupBy(
                        rep => new { rep.MarketId, Mic = rep.MarketIdentifierCode, rep.MarketName, rep.Epoch },
                        (key, group) => new
                                            {
                                                Key1 = key.MarketId,
                                                Key2 = key.MarketName,
                        ...
                        i =>
                            {
                                var market = new Market(i.Key1, i.Key4, i.Key2, MarketTypes.STOCKEXCHANGE);
                                var frame = new EquityInterDayTimeBarCollection(
                                    market,
                                    i.Key3,
                                    i.Result.Select(o => this.ProjectToInterDaySecurity(o, market)).ToList());
            ...
            {
                this._logger.LogError(
                    $"ReddeerMarketRepository GetEquityInterDay Method For {start} {end} {e.Message}");
                opCtx.EventError(e);
            }
            finally
            {
                this._logger.LogInformation("ReddeerMarketRepository GetEquityInterDay closed connection to db");
            ...
            ISystemProcessOperationContext opCtx)
        {
            ...
            {
                this._logger.LogWarning(
                    $"ReddeerMarketRepository Get request had a start date larger than end date {start} {end}");
            ...
            {
                dbConnection.Open();
                this._logger.LogInformation("ReddeerMarketRepository get opened connection to db");

                this._logger.LogInformation($"ReddeerMarketRepository get about to query for {start} and {end}");
                using (var conn =
                    dbConnection.QueryAsync<MarketStockExchangeSecuritiesDto>(GetEquityIntraDaySql, query))
                {
                    var response = (await conn)?.ToList() ?? new List<MarketStockExchangeSecuritiesDto>();

                    this._logger.LogInformation(
                        $"ReddeerMarketRepository get query for {start} and {end} received {response?.Count() ?? 0} results");
                    var groupedByExchange = response.GroupBy(
                        rep => new { rep.MarketId, Mic = rep.MarketIdentifierCode, rep.MarketName, rep.Epoch },
                        (key, group) => new
                                            {
                        ...
                        i =>
                            {
                                ...
                                var frame = new EquityIntraDayTimeBarCollection(
                                    market,
                                    i.Key3,
                                    i.Result.Select(o => this.ProjectToSecurity(o, market))
                                        .Where(_ => _.SpreadTimeBar.Price.Value > 0).ToList());
            ...
            {
                this._logger.LogError($"ReddeerMarketRepository Get Method For {start} {end} {e.Message}");
                opCtx.EventError(e);
            }
            finally
            {
                this._logger.LogInformation("ReddeerMarketRepository get closed connection to db");
        ...
        public async Task<IReadOnlyCollection<SecurityEnrichmentDto>> GetUnEnrichedSecurities()
        {
            ...
            try
            {
                this._logger.LogInformation("ReddeerMarketRepository opening db connection for unenriched securities");

                dbConnection.Open();
                this._logger.LogInformation("ReddeerMarketRepository getting unenriched securities");
                using (var conn = dbConnection.QueryAsync<SecurityEnrichmentDto>(GetUnEnrichedSecuritiesSql))
                {
                    var result = await conn;

                    this._logger.LogInformation(
                        $"ReddeerMarketRepository returning {result?.Count() ?? 0} unenriched securities");
                    return result?.ToList();
                }
            }
            catch (Exception e)
            {
                this._logger.LogError($"ReddeerMarketRepository GetUnEnrichedSecurities method for {e.Message}");
            }
            finally
            {
                this._logger.LogInformation("ReddeerMarketRepository closing db connection for unenriched securities");
        ...
        public async Task UpdateUnEnrichedSecurities(IReadOnlyCollection<SecurityEnrichmentDto> dtos)
        {
            ...
            {
                this._logger.LogInformation("ReddeerMarketRepository update unenriched securities received none");
            ...
            try
            {
                dbConnection.Open();
                this._logger.LogInformation(
                    "ReddeerMarketRepository opening db connection in update unenriched securities");

                var projectedDtos = dtos.Select(dto => new SecurityEnrichmentDtoDapper(dto, this._cfiMapper)).ToList();

                this._logger.LogInformation(
                    $"ReddeerMarketRepository about to update {projectedDtos.Count} unenriched securities");
                using (var conn = dbConnection.ExecuteAsync(UpdateUnEnrichedSecuritiesSql, projectedDtos))
                {
                    await conn;
                    this._logger.LogInformation("ReddeerMarketRepository completed updating unenriched securities");
                }
            }
            catch (Exception e)
            {
                this._logger.LogError($"ReddeerMarketRepository UpdateUnEnrichedSecurities method for {e.Message}");
            }
            finally
            {
                this._logger.LogInformation(
                    "ReddeerMarketRepository closing db connection for update unenriched securities");
        ...
        {
            return new InsertSecurityDto(tick, null, this._cfiMapper);
        }
            ...
            Market market)
        {
            ...
            var tick = new EquityInstrumentInterDayTimeBar(security, dailySummary, dto.Epoch, market);

            return tick;
        }

        private EquityInstrumentIntraDayTimeBar ProjectToSecurity(MarketStockExchangeSecuritiesDto dto, Market market)
        {
            if (dto == null) return null;
            ...
            var tick = new EquityInstrumentIntraDayTimeBar(security, spread, dailySummary, dto.Epoch, market);
        ...
        public class SecurityEnrichmentDtoDapper
        {
            ...
            public SecurityEnrichmentDtoDapper(SecurityEnrichmentDto dto, ICfiInstrumentTypeMapper cfiMapper)
            {
                ...
                if (cfiMapper == null) throw new ArgumentNullException(nameof(cfiMapper));
        ...
        {
            public InsertSecurityDto(
                EquityInstrumentIntraDayTimeBar entity,
                string marketIdForeignKey,
                ICfiInstrumentTypeMapper cfiMapper)
            {
                ...
                this.SecurityCurrency = entity.SpreadTimeBar.Price.Currency.Code
                ...
                this.MarketCap = entity.DailySummaryTimeBar.MarketCapCents;
            ...
            public string MarketIdPrimaryKey { get; }
            ...
            public string SecurityCurrency { get; }
            ...
            public string UnderlyingCfi { get; }
        ...
        private class MarketStockExchangeDto
        {
            ...
            public MarketStockExchangeDto(EquityIntraDayTimeBarCollection entity)
            {
                ...
                this.MarketId = entity?.Exchange?.MarketIdentifierCode;
                this.MarketName = entity?.Exchange?.Name;
            }
        ...
        private class MarketStockExchangeSecuritiesDto
        {
            ...
            public string ReddeerEnrichmentId { get; }
        ...
        private class MarketUpdateDto
        {
            ...
            public MarketUpdateDto(Market market)
            {
                this.MarketId = market.MarketIdentifierCode;
