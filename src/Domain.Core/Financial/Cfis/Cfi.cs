namespace Domain.Core.Financial.Cfis
{
    ...
    public class Cfi
    {
        ...
        {
            this.Value = underlyingValue?.ToLower();

            if (string.IsNullOrWhiteSpace(this.Value)) return;

            this.CfiCategory = this.InferCategory(this.Value);
            this.CfiGroup = this.InferGroup(this.Value, this.CfiCategory);
        }
        ...
        {
            if (string.IsNullOrWhiteSpace(cfi)) return CfiCategory.None;
        ...
        {
            if (cfi.Length < 2) return CfiGroup.None;
        ...
        {
            if (string.IsNullOrWhiteSpace(cfi) || cfiCategory == CfiCategory.None) return CfiGroup.None;
